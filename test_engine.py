...
def min_maxN_pruned(BOARD, N, alpha=float('-inf'), beta=float('inf')):
    ...
    if board_hash in transposition_table and transposition_table[board_hash][0] >= N:
        return transposition_table[board_hash][1]  # Return stored best move
    opening_move = reader.get(BOARD)
    if opening_move:
        print("Using book")
        return opening_move.move
    ...
    if BOARD.turn:  # White to move (Maximizing)
        ...
        for move in moves:
            ...
            if beta <= alpha:
                break  # Alpha cut-off
    transposition_table[board_hash] = (N, best_move)
    return best_move

def test_timing_and_output(BOARD, depth):
    start_time = time.time()
    result_pruned = min_maxN_pruned(BOARD, depth)
    time_pruned = time.time() - start_time

    start_time = time.time()
    result_unpruned = min_maxN(BOARD, depth)
    time_unpruned = time.time() - start_time

    assert result_pruned == result_unpruned, f"Results do not match!\n{result_pruned, result_unpruned}\n{BOARD}"
    print(f"Pruned Time: {time_pruned:.5f}s, Unpruned Time: {time_unpruned:.5f}s")

for i in range(10000):
    b = chess.Board()
    for i in range(10):
        moves = list(b.legal_moves)
        b.push(moves[random.randint(0,1)])
    test_timing_and_output(b, 3)